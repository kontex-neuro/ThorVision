...
def parse_filename(filename):
    match = re.match(r"render_(\d+)_(\d+)_(\d+)\.log", filename)
    if match:
...
def collect_log_data(folder_path):
    ...
    for filename in os.listdir(folder_path):
        if filename.startswith("render_") and filename.endswith(".log"):
...
def plot_fps_data(data, log_scale=True):
    ...
    plt.xlabel("Render Instances")
    plt.ylabel("Average FPS (log scale)" if log_scale else "Average FPS")
    if log_scale:
        plt.yscale("log")
    plt.title("Render Performance vs Instance Count")
    plt.legend()
...
def main(folder, linear):
    """
    Visualize render performance logs in FOLDER.
